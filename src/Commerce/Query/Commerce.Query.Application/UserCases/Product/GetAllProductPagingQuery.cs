...
namespace Commerce.Query.Application.UserCases.Product
{
    ...
    /// </summary>
    public class GetAllProductPagingQuery : IRequest<Result<PaginatedResult<ProductDTO>>>
    {
        public int PageNumber { get; set; }
        public int PageSize { get; set; } = 5;

        public GetAllProductPagingQuery(int pageNumber)
        {
    ...
    /// </summary>
    public class GetAllProductPagingQueryHandler : IRequestHandler<GetAllProductPagingQuery, Result<PaginatedResult<ProductDTO>>>
    {
        ...
        public async Task<Result<PaginatedResult<ProductDTO>>> Handle(GetAllProductPagingQuery request, CancellationToken cancellationToken)
        {
            int pageNumber = request.PageNumber;
            int pageSize = request.PageSize;

            // Định nghĩa predicate (có thể áp dụng thêm điều kiện tìm kiếm nếu cần)
            Expression<Func<Entities.Product, bool>> predicate = product => true;
